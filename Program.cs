    ...
    class Program
    {
        ...
        */
        ...
        static void NorwayFlag(int flatPerFloor, int padikiCount, int floorCount)
        {
            ...
            Console.WriteLine($"flats In Houses:{flatPerFloor * floorCount * padikiCount}");
            //Нада ебануть проверку на минимальный размер флага
            if (floorCount < 16 || padikiCount * flatPerFloor < 27)
            {
                Console.WriteLine("Minimum flag size is 16*27");
            }
            //проверка на не горящие квартиры
            int noLitColomn = 0;
            int noLitFloor = 0;

            if (floorCount / 16 == padikiCount * flatPerFloor / 27 && floorCount % 16 == 0 && padikiCount * flatPerFloor % 27 == 0)
            {
                Console.WriteLine("All right bitches. Light it all.");
            }
            if (floorCount % 16 != 0)
            {
                if (floorCount / 16 > padikiCount * flatPerFloor / 27)
                {
                    noLitFloor = floorCount - ((padikiCount * flatPerFloor) / 27) * 27;
                }
                else
                {
                    noLitFloor = floorCount - ((floorCount) / 16) * 16;
                }
                Console.WriteLine($"Your first {noLitFloor} floor(s) is(are) not lit");
            }
            if (padikiCount * flatPerFloor % 27 != 0)
            {
                if (floorCount > padikiCount * flatPerFloor)
                {
                    noLitColomn = padikiCount * flatPerFloor - ((floorCount) / 16) * 16;
                }
                else
                {
                    noLitColomn = padikiCount * flatPerFloor - ((padikiCount * flatPerFloor) / 27) * 27;
                }
                Console.WriteLine($"Your first {noLitColomn} colomn(s) is(are) not lit");
            }
            if (floorCount % 16 == 0 && padikiCount * flatPerFloor % 27 == 0 && floorCount > padikiCount * flatPerFloor)
            {
                noLitFloor = floorCount - padikiCount * flatPerFloor;

                Console.WriteLine($"Your first {noLitFloor} floor(s) is(are) not lit");
            }
            if (floorCount % 16 == 0 && padikiCount * flatPerFloor % 27 == 0 && floorCount/16 < padikiCount * flatPerFloor/27)
            {
                noLitColomn = padikiCount * flatPerFloor - floorCount;
                ...
                for (; indexX <= padikiCount * flatPerFloor; indexX++)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ((indexY <= (floorCount - noLitFloor) * 6 / 16 + noLitFloor) || (indexY > (floorCount - noLitFloor) * 10 / 16 + noLitFloor))
                            {
                                if ((indexX <= (padikiCount * flatPerFloor - noLitColomn) * 7 / 27 + noLitColomn) || (indexX > (padikiCount * flatPerFloor - noLitColomn) * 11 / 27 + noLitColomn))
                                {
                                    Console.ForegroundColor = ConsoleColor.Red;
                                    Console.Write("*");
                                }
                                else if ((indexX > (padikiCount * flatPerFloor - noLitColomn) * 8 / 27 + noLitColomn) && (indexX <= (padikiCount * flatPerFloor - noLitColomn) * 10 / 27 + noLitColomn))
                                {
                            ...
                            else if ((indexY > (floorCount - noLitFloor) * 7 / 16 + noLitFloor) && (indexY <= (floorCount - noLitFloor) * 9 / 16 + noLitFloor))
                            {
                            ...
                            {
                                if ((indexX > (padikiCount * flatPerFloor - noLitColomn) * 8 / 27 + noLitColomn) && (indexX <= (padikiCount * flatPerFloor - noLitColomn) * 10 / 27 + noLitColomn))
                                {
                                    Console.ForegroundColor = ConsoleColor.DarkBlue;
                                    Console.Write("*");
                                }
